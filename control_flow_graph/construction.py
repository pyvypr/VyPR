	...
	if type(obj) is ast.Name:
		...
		if omit_subscripts:
			...
			if type(obj.slice.value) is ast.Str:
				return ["[\"%s\"]" % obj.slice.value.s] + get_reversed_string_list(obj.value, omit_subscripts=omit_subscripts)
			elif type(obj.slice.value) is ast.Num:
				return ["[%i]" % obj.slice.value.n] + get_reversed_string_list(obj.value, omit_subscripts=omit_subscripts)
			elif type(obj.slice.value) is ast.Name:
				return ["[%s]" % obj.slice.value.id] + get_reversed_string_list(obj.value, omit_subscripts=omit_subscripts)
		...
		if not(entry):
			...
			if type(entry) is ast.Assign and type(entry.value) in [ast.Call, ast.Expr]:
				...
				if type(entry.targets[0]) is ast.Tuple:
					self._name_changed = map(get_attr_name_string, entry.targets[0].elts) + get_function_name_strings(entry)
				else:
...
class CFG(object):
		...
		for (n, entry) in enumerate(block):
			...
			if type(entry) is ast.Pass:
				...
				current_vertices = filter(
					lambda vertex : vertex._previous_edge is None or not(type(vertex._previous_edge._instruction) in [ast.Return, ast.Raise]),
					current_vertices
				)
	...
	def derive_grammar(self):
		...
		for vertex in self.vertices:
			...
			if len(vertex.edges) == 0:
			...
			elif not(vertex._name_changed in [["conditional"], ["loop"], ["try-catch"], ["post-conditional"], ["post-loop"], ["post-try-catch"]]):
				...
				if not(vertex.edges[0]._target_state._name_changed in [["conditional"], ["try-catch"]]):
					...
					if vertex.edges[0]._target_state._name_changed in [["post-conditional"], ["post-try-catch"]]:
						...
						loop_skip_edge = filter(lambda edge : edge._target_state._name_changed != ["loop"], vertex.edges)[0]
				...
				loop_entry_edge = filter(lambda edge : edge._target_state._name_changed != ["post-loop"], vertex.edges)[0]
		...
		if not(vertex in marked_vertices):
			...
			for edge in edges:
				if ((type(edge._instruction) is ast.Expr
					...
					and type(edge._instruction.value) is ast.Call and function in get_function_name_strings(edge._instruction.value))):
					calls.append(edge)
				else:
	...
	def process_required_variables_list(self, required):
		"""
		Note: the current treatment of Lambdas causes any variables
		used in their body to be a free variable.
		"""
		result = []
		for var in required:
			if type(var) in [ast.Dict, ast.Lambda, ast.arguments, ast.Param]:
				# for now, we don't explicitly process these types
				# just what's inside them
				continue
			name = get_attr_name_string(var, omit_subscripts=False)
			if not(name in [None, "self"]):
				if "." in name:
					split = name.split(".")
					if len(split) > 1 and split[-2] != "self":
						name = ".".join(name.split(".")[:-1])
				if not(name in result):
					result.append(name)
		return result

	def find_initial_affecting_vertices(self, edge, free_variables):
		#all_variables = map(lambda item : item.id, edge._instruction.value.args)
		print("starting by processing the arguments:", edge._instruction.value.args)
		all_variables = self.process_required_variables_list(edge._instruction.value.args)
		print("starting traversal with %s" % all_variables)
		return self.find_initial_affecting_vertices_recurse(edge._source_state, all_variables, l=0, free_variables=free_variables)

	def find_initial_affecting_vertices_recurse(self, current_vertex, variables_required=[], l=0, free_variables=[]):
		"""
		Recursively determine the initial affecting variables.
		Each path generating by the recursion will maintain a separate list of required variables.
		"""
		indent = "  "*l
		independent_variables = ["sys", "random", "dict", "map"]
		print("%sprocessing vertex %s" % (indent, current_vertex))
		# copy the list so we're not modifying any other path's list
		variables_required_copy = [v for v in variables_required]
		print("%svariables required are %s" % (indent, variables_required_copy))
		# find edges who target state is current_vertex
		relevant_edges = filter(lambda edge : edge._target_state == current_vertex and type(edge._instruction) is ast.Assign, self.edges)
		print("%srelevant edges are %s" % (indent, relevant_edges))
		if len(relevant_edges) == 0:
			# no assignment edges found, so check for control flow edges
			# TODO: control flow is not the only alternative for kinds of edges - we will have to deal with different kinds of dependence
			relevant_edges = filter(lambda edge : edge._target_state == current_vertex and (edge._instruction in [None, "control-flow"] or type(edge._instruction) is ast.Print), self.edges)
			print("%srelevant (non-assignment) edges are %s" % (indent, relevant_edges))
			if len(relevant_edges) == 0:
				# we reached the start state
				print("%sfinal state reached" % indent)
				# copy the required variables into the list of free variables
				for var in variables_required_copy:
					if not(var in free_variables):
						free_variables.append(var)
				print("%sfree variables are %s" % (indent, free_variables))
				# return an empty list of affecting vertices, since
				# we've now concluded that every affecting vertex is free
				return []
			else:
				# we're at a state where there are no incident assignments,
				# so recurse on each incident edge
				affecting_vertices = []
				for relevant_edge in relevant_edges:
					affecting_vertices += self.find_initial_affecting_vertices_recurse(relevant_edge._source_state, variables_required_copy, l+1, free_variables)
				return affecting_vertices
		else:
			# assignments have been found
			print("%sprocessing relevant edges" % indent)
			affecting_vertices = []
			for n, relevant_edge in enumerate(relevant_edges):
				print("%sprocessing edge %s" % (indent, relevant_edge))
				# check if this assignment contributes to any of the variables we've seen so far
				#print(get_attr_name_string(relevant_edge._instruction.targets[0]))
				obj_as_str = get_attr_name_string(relevant_edge._instruction.targets[0])
				#if type(relevant_edge._instruction.targets[0]) is ast.Name and relevant_edge._instruction.targets[0].id in variables_required_copy:
				if obj_as_str in variables_required_copy:
					# this contributes to a variable we care about
					print("%sprocessing assignment for %s" % (indent, obj_as_str))

					# we've observed the change, so for now stop checking for assignments that change this
					variables_required_copy.remove(obj_as_str)
					# for dictionaries or arrays, we need to check if we've encountered the definition of the original structure
					for (n, var) in enumerate(variables_required_copy):
						if "[" in var and "]" in var and obj_as_str == var[0:var.index("[")]:
							del variables_required_copy[n]

					# now, see if there are any variables we can follow that are used to define this
					# irrelevant types are a set of objects we have no need to process
					irrelevant_types = [ast.Str, ast.Index, ast.Load, ast.Num, ast.keyword]
					chain = filter(
						lambda item : not(type(item) in irrelevant_types) and not(type(item) is ast.Name and item.id is "None"),
						list(ast.walk(relevant_edge._instruction.value))
					)
					all_variables = self.process_required_variables_list(filter(lambda item : not(item) is None, chain))

					# for every variable that's a subscript, we need to remove the reference to the list of which it is a subscript
					for (n, var) in enumerate(all_variables):
						if "[" in var and "]" in var:
							for (m, other) in enumerate(all_variables):
								if var != other:
									if other == var[0:var.index("[")]:
										# "other" is more general than we ne
										del all_variables[m]

					print("%sall variables found in this expression are %s" % (indent, all_variables))
					difference = list(set(all_variables)-set(independent_variables))
					if len(difference) > 0:
						# there are variables we can follow
						print("%sthere are variables we can follow: %s" % (indent, difference))
						for variable in difference:
							if not(variable in variables_required_copy):
								variables_required_copy.append(variable)
						affecting_vertices += self.find_initial_affecting_vertices_recurse(relevant_edge._source_state, variables_required_copy, l+1, free_variables)
					else:
						# there are no variables to follow
						print("%sno variables we can follow - this is an affecting vertex" % indent)
						# return this vertex without traversing further
						if len(variables_required_copy) > 0:
							print("%swe still have to find definitions of %s - carrying on with traversal" % (indent, variables_required_copy))
							affecting_vertices += self.find_initial_affecting_vertices_recurse(relevant_edge._source_state, variables_required_copy, l+1, free_variables) + [current_vertex]
						else:
							print("%sno more variables to find on this path - ending traversal" % indent)
							affecting_variables += [current_vertex]
				else:
					# this vertex didn't define anything we need - ignore it and recurse
					affecting_vertices += self.find_initial_affecting_vertices_recurse(relevant_edge._source_state, variables_required_copy, l+1, free_variables)
			# return the final list of affecting vertices from recursion inside the for loop
			return affecting_vertices


	"""def find_initial_affecting_vertices(self, edge, starting_variable):
		independent_variables = ["sys"]
		vertex_stack = [edge._source_state]
		#chain = list(ast.walk(edge._instruction.value.args))
		#all_variables = map(lambda item : item.id, filter(lambda item : type(item) is ast.Name, chain))
		all_variables = map(lambda item : item.id, edge._instruction.value.args)
		awaited_changes = all_variables
		print("variable changes that must be observed are %s" % awaited_changes)
		final_vertices = []
		while len(vertex_stack) > 0 and len(awaited_changes) > 0:
			top = vertex_stack.pop()
			print("Processing top of stack:")
			print(top)
			print("\t awaited changes are %s" % awaited_changes)
			# find edges whose target state is top
			relevant_edges = filter(lambda edge : edge._target_state == top and type(edge._instruction) is ast.Assign, self.edges)
			if len(relevant_edges) == 0:
				# the only edge must be a control flow edge
				relevant_edges = filter(lambda edge : edge._target_state == top and edge._instruction in [None, "control-flow"], self.edges)
				if len(relevant_edges) == 0:
					# we reached the start state
					print("final state reached")
					print(awaited_changes)
					break
				else:
					vertex_stack.append(relevant_edges[0]._source_state)
			else:
				print("\tProcessing relevant edges %s" % relevant_edges)
				for relevant_edge in relevant_edges:
					# check if this assignment contributes to any of the variables we've seen so far
					if relevant_edge._instruction.targets[0].id in awaited_changes:
						print("processing assignment for %s" % relevant_edge._instruction.targets[0].id)
						# this contributes to a variable we care about
						# we've observed the change, so for now stop checking for assignments that change this
						awaited_changes.remove(relevant_edge._instruction.targets[0].id)
						# now, see if there are any variables we can follow that are used to define this
						chain = list(ast.walk(relevant_edge._instruction.value))
						all_variables = map(lambda item : item.id, filter(lambda item : type(item) is ast.Name, chain))
						difference = list(set(all_variables)-set(independent_variables))
						if len(difference) > 0:
							print("\t there are variables we can follow: %s" % difference)
							# there are variables we can follow
							for variable in difference:
								if not(variable in awaited_changes):
									awaited_changes.append(variable)
						else:
							print("\t no variables we can follow - this is an affecting vertex")
							# there are no variables we can follow, so we've found a final vertex
							#for changed_variable in top._target_state._name_changed:
							#	if changed_variable in awaited_changes:
							final_vertices.append(top)

					# add the previous state wrt this edge to the stack
					vertex_stack.append(relevant_edge._source_state)
